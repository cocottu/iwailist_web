...
- 表示項目:
  - サービス名
  - **運営者名（「cocottu」）**
  - **「本サービスは cocottu による個人開発プロジェクトです。」と明記**
  - 連絡先（お問い合わせページへのリンク）
  - 所在地（必要に応じて）
  - 免責事項へのリンク
...
**UI/UX 方針**:
- セクション見出しを明確にする（例: 「運営者について」「本サービスについて」「連絡先」など）
- 冒頭で「cocottu 個人開発によるサービス」であることがひと目で分かるようにテキストを配置
- モバイルで読みやすい余白と行間
- 法的文言はできるだけ平易な日本語で補足
...
**送信方法の方針（Slack Webhook）**:
- **お問い合わせ内容は Slack の Incoming Webhook に投稿する** 方式とする
- フロントエンドから **環境変数に保存された Webhook URL を使用して HTTP POST**（`fetch`）を実行
  - 送信ペイロード例: チャンネル・ユーザー名・本文を含む JSON（`text` に整形済みメッセージ）
  - Webhook URL は `import.meta.env.VITE_SLACK_CONTACT_WEBHOOK_URL` などで参照し、**ハードコーディング禁止**
- 送信前に確認ダイアログ（モーダル）で内容を表示し、「送信」クリックで Slack 連携を実行
- 送信成功時は完了メッセージ表示、失敗時はリトライ案内付きエラー表示

**将来フェーズでの選択肢**:
- Firebase Functions 経由で Slack へ送るプロキシ API を用意し、Webhook URL をクライアントから隠蔽
- Firestore への問い合わせ履歴保存（Slackと二重記録）
